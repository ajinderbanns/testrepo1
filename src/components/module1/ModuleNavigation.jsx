 ...
 * 
 * Navigation sidebar/list for Module 1 sections with progress indicators,
...
import { MODULE_1_SECTION_STRUCTURE } from '../../data/module1Structure'

/**
 * ModuleNavigation Component
 * 
 * @param {Object} props
 * @param {string} props.currentPath - Current section path
...
function ModuleNavigation({ 
  ...
  const { isSectionCompleted, completionPercentage } = useModuleProgress(1)

  const handleSectionClick = (section) => {
    const newPath = `/module/1/${section.path}`
    navigate(newPath)
    if (onSectionChange) {
  ...
  return (
    <nav className={`module-navigation ${className}`} style={containerStyles}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
        {MODULE_1_SECTION_STRUCTURE.map((section) => {
