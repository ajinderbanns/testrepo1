/**
 * ModuleNavigation Component for Module 2
 * 
 * Navigation sidebar/list for Module 2 sections with progress indicators,
...
import { MODULE_2_SECTION_STRUCTURE } from '../../data/module2Structure'
...
function ModuleNavigation({ 
  ...
  const { isSectionCompleted, completionPercentage } = useModuleProgress(2)

  const handleSectionClick = (section) => {
    const newPath = `/module/2/${section.path}`
    navigate(newPath)
    if (onSectionChange) {
  ...
  return (
    <nav className={`module-navigation ${className}`} style={containerStyles}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
        {MODULE_2_SECTION_STRUCTURE.map((section) => {
